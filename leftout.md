...
Current audit: **14 September 2026**. This is the application's current progress tracker. [featureDevelopmentInfo.md](featureDevelopmentInfo.md) explains the implementation; [requirements.md](requirements.md) supplies numbered requirements; [VALIDATION.md](VALIDATION.md) retains dated test/build evidence. The 11 September PDF reconciliation implementation includes new tests, real-layout validation, schema migration and APK rebuilds; see the dated validation record.

The core ledger, encryption, manual tracking, ingestion/reconciliation and backup flows exist. **The app is not fully release-validated:** the supplied SBI Relationship Summary layout is validated, while other PDF layouts, notification adapters, unseen SMS formats/device behavior and the gaps below still require work.
...
| SMS | [MessageParsers.kt](app/src/main/java/com/kiz9r/expense_tracker/ingestion/MessageParsers.kt), [SmsFields.kt](app/src/main/java/com/kiz9r/expense_tracker/ingestion/SmsFields.kt), [SmsIntake.kt](app/src/main/java/com/kiz9r/expense_tracker/ingestion/SmsIntake.kt), [IngestionServices.kt](app/src/main/java/com/kiz9r/expense_tracker/ingestion/IngestionServices.kt). |
| SMS-XML | [ProvidedSmsXmlTest.kt](app/src/test/java/com/kiz9r/expense_tracker/ProvidedSmsXmlTest.kt), opt-in secure host SAX audit of 299 actual SBI messages; [SbiObservedLayoutsTest.kt](app/src/test/java/com/kiz9r/expense_tracker/SbiObservedLayoutsTest.kt), 11 invented-value regression groups. |
| SMS-TEST | [SmsParserTest.kt](app/src/test/java/com/kiz9r/expense_tracker/SmsParserTest.kt) (12 JVM); [SmsIntegrationTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/SmsIntegrationTest.kt) (12 Room); [SmsReviewUiTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/SmsReviewUiTest.kt) and [SmsReceiverDeliveryTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/SmsReceiverDeliveryTest.kt) (Compose and actual emulator multipart/repeated/OTP broadcast). |
...
| BACKUP-TEST | [BackupArchiveTest.kt](app/src/test/java/com/kiz9r/expense_tracker/BackupArchiveTest.kt), [BackupRecoveryTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/BackupRecoveryTest.kt), [DeviceKeyRecoveryTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/DeviceKeyRecoveryTest.kt), [BackupUiTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/BackupUiTest.kt), [BackupReinstallTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/BackupReinstallTest.kt) and its [emulator fixture](app/src/androidTest/backup-reinstall-fixture.py). |
...
| RUN | [VALIDATION.md](VALIDATION.md); historical PDF run includes private-sample validation. Latest 14 September backup/recovery run: 47 JVM passes plus one optional XML skip. Android runner OK (65 tests): 62 executed passes and three opt-in skips; actual emulator reinstall then passed both phases as a separate distinct test. Earlier private-SMS/PDF and receiver-broadcast passes remain dated evidence. Lint zero errors / 16 warnings; signed version 1.3/code 5 built and verified. No physical phone sign-off. |
...
| ID / feature | Requirement / plan | Scope | Implementation | Validation | Completed work | Remaining work | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- |
| F024 SBI SMS formats and permission flow | §§19–21,43–46,54–55; M3 | MVP | Partial | Recorded pass: SMS-XML, SMS-TEST and RUN; observed layouts, synthetic regressions and earlier emulator SMS delivery | Version 2.1 modular classifiers, validated UPI/NEFT/IMPS/card/CBS/NACH/cash/cheque/mandate layouts, UMN/UMRN identity, service-message exclusions, strict fields and explicit review; incoming-only permission/unlock gates and Settings status. | Supplied 299-message SBI corpus validated for its observed layouts; seven card-only alerts correctly need account selection. Other ATM/fees/refund/reversal formats absent from it remain synthetic-only. Phone delivery/permission/OEM QA pending. | Validate additional source families as examples become available; retain the corpus audit and conservative review fallback. |
| F025 Idempotent background processing | §§21,46,53,59,65; M3/M5 | MVP | Completed | Recorded pass: SMS-TEST multipart/intake/database and opt-in actual emulator broadcast; RUN | Same-sender bounded assembly; stable sender-time/content identities; encrypted transactional intake; bounded caught receiver failures; stored malformed events cannot block queue; startup/boot-after-unlock/update/manual retry recovery. | Physical reboot/force-stop/OEM guarantees remain unverified; pre-unlock or never-delivered messages require statement recovery. | Retain passing delivery regressions; verify physical lifecycle and supplied real formats later. |
...
| F027 Mandates and execution | §§20,30,40; M3/M7 | MVP | Partial | Recorded pass: supplied XML create/cancel/UMRN classification plus synthetic JVM/ROOM lifecycle and non-spending checks | Separate mandate records/list, creation/cancellation status, execution debit classification, reminder held unknown; event-time ordering protects cancellation from late creation. | Paused/expired/unknown lifecycle, schedule/frequency display and execution-to-mandate linkage absent; synthetic cancellation/late-create/execution passes in SMS-TEST; 25 actual mandate messages and 30 reminder/collect requests validated from the supplied corpus; wider lifecycle states remain unimplemented. | Define full mandate lifecycle and add create/cancel/execute flow tests. |
...
| F032 App lock and screenshot protection | §§3,41,43,67; M6 | MVP | Partial | Recorded pass: BACKUP-TEST isolated key-loss recovery; SEC/UI inspected; physical app-lock checks pending | Optional Android biometric/device credentials, onStop relock, fail-closed loading, FLAG_SECURE default and configurable screenshots, first-unlock-independent ingestion; startup recovery can authenticate a backup without the lost database key, retain original files and activate a fresh encrypted ledger. | Physical lock lifecycle/enrollment/SAF return testing pending; missing-key recovery now stages a separate encrypted ledger and retains original files; physical hardware/key-loss lifecycle checks remain pending. | Verify the implemented recovery screen and lock lifecycle on the phone. |
| F033 Portable encrypted backup export | §§41–42; M6 | MVP | Completed | Recorded pass: BACKUP-TEST all-record archive/password/tampering/output-failure checks; JVM crypto and ROOM restore tests | Versioned full logical snapshot excluding transient jobs/device keys, AES-GCM authenticated header, PBKDF2, password policy and SAF output. | Large archive/provider behavior and memory scaling remain F037/F038. | Retain BACKUP-TEST full-record fixture; profile large archives and check physical SAF providers. |
| F034 Archive validation and replacement restore | §§42,54,57; M6 | MVP | Completed | Recorded pass: BACKUP-TEST full archive/validation/rollback/concurrency/onboarding flow; ROOM and PDF-ROOM regressions | Versions 1/2 restore; strict JSON fields/types and financial relationships checked before preview; onboarding restore, masked account/count preview and confirmation; cancellation/write-failure rollback; SMS and notification opt-in checked transactionally with writes. | Physical recovery/SAF/interruption checks remain F035/F038; archive memory profiling remains F037. | Retain rollback and relationship checks; complete physical recovery validation under F035. |
| F035 Uninstall/reinstall recovery | §§42,58,72 scenario 12; M6 | Required release validation | Partial | Recorded pass: BACKUP-TEST actual Android 17 emulator uninstall/reinstall, changed database-key fingerprint and complete record digest | Encrypted fixture exported, app uninstalled/reinstalled, fresh SQLCipher key generated, every exported record restored; SMS/notifications disabled afterward. | Physical phone/replacement-device recovery, SAF providers and permission reestablishment still need QA. | Use a disposable synthetic ledger on a physical device; record dated recovery results. |
...
| F039 Automated acceptance and edge-case suite | §§58–59,72; all milestones | MVP validation | Partial | Recorded pass: RUN current unit/emulator suite plus real-SMS/PDF historical validation and new full-record recovery, rollback and reinstall tests | New layout, 120-row and page-order fixtures, repeated/ignored overlaps, concurrency, drafts, cancellation, warnings/backup, encrypted migration and refund dates covered. | Remaining unseen-source, physical-device, large-archive and performance coverage stays explicit. | Retain the PDF run and continue broader device/source acceptance checks. |
| F040 Release APK and installation | §§2,70–71; M7 | MVP deliverable | Partial | Recorded pass: RUN version 1.3 debug/test install, release assembly and v2 signing verification; unit/emulator and actual reinstall proof passed | Optimized signed APK with schema-2 migration; README and supported PDF layout updated. | 16 lint warnings remain; physical install/update, recovery and accessibility are unverified (F035/F038). | Verify phone install/update/recovery and resolve actionable warnings. |
...
| --- | --- | --- | --- | --- |
| A01 — Debit SMS creates one provisional transaction | F018, F024–F025 | SMS parser -> enqueue/processPending -> provisional canonical record | Recorded pass: SMS-TEST actual long multipart and repeated emulator SMS reaches the encrypted ledger as one provisional transaction; intake unit/Room retries also pass | Verify actual carrier-delivered SBI messages and lifecycle behavior on the phone. |
...
| A06 — Mandate creation does not create spending | F027 | Separate mandate entity; no spending movement on authorization | Recorded pass: ROOM.failedPaymentsAndMandatesDoNotIncreaseSpending and JVM classification | Observed creation/cancellation/UMRN wording validated; synthetic late-create/execution passes. Wider lifecycle states remain pending. |
| A07 — Failed payment does not increase spending | F026 | Failed outcome excluded; statement can later confirm posted | Recorded pass: ROOM failed/mandate totals and verifiedStatementCanConfirmPreviouslyFailedPayment | Scheduled/collect-request wording is validated; real failed/refund/reversal wording remains pending. |
...
| A12 — Encrypted backup survives uninstall/reinstall | F033–F035 | Portable encryption and transactional replacement restore | Recorded pass on Android 17 emulator: actual uninstall/reinstall, changed database key, all-record digest and disabled tracking; physical phone still pending | Restore every persistent entity type on a physical fresh install and compare; record key/permission behavior. |
...
| M3 Incoming SMS | Version 2.1 observed-layout parsing, explicit review and actual emulator multipart delivery tested | Observed source families in F024 are validated; unseen formats and phone delivery/permission/background QA F038 remain. |
...
| M6 Security/backup | Encryption and core export/restore implemented | Physical recovery/app-lock/SAF validation F032/F035/F038; automated archive/reinstall checks pass. |
...
1. Preserve the validated Relationship Summary layout and its passing PDF-UI regression; retain the observed SMS corpus audit and expand unseen formats (F024). Keep real records private and commit only synthetic equivalents.
2. Retain complete archive validation/recovery regressions and prove physical reinstall recovery (F032/F034/F035).
