...
| ROOM | [LedgerIntegrationTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/LedgerIntegrationTest.kt); 15 repository/integration tests recorded passed, chiefly in-memory Room. |
...
| RUN | [VALIDATION.md](VALIDATION.md); historical 10 September run plus 11 September PDF run: 21 JVM / 36 Android tests passed, lint zero errors / 16 warnings, signed version 1.1 APK verified. No physical phone sign-off. |
...
| F006 Account maintenance | §7; M1 | MVP refinement | Partial | Code inspected: DB, LEDGER, UI | Active flag exists; account creation/list/selection works. | No account rename/type-edit/deactivation UI or updatedAt field. | Add safe maintenance flow preserving historical account ownership. |
