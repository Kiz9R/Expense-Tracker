...
| [data/LedgerRepository.kt](app/src/main/java/com/kiz9r/expense_tracker/data/LedgerRepository.kt) | Validation, atomic account erasure, manual CRUD, metadata/tags, categories, rules, settings, history and SaveManualTransaction. |
...
| [ingestion/SmsIntake.kt](app/src/main/java/com/kiz9r/expense_tracker/ingestion/SmsIntake.kt) / [SmsFields.kt](app/src/main/java/com/kiz9r/expense_tracker/ingestion/SmsFields.kt) | Incoming-only eligibility/assembly/persistence and conservative financial field extraction. |
...
| [backup/BackupArchive.kt](app/src/main/java/com/kiz9r/expense_tracker/backup/BackupArchive.kt) and [backup/BackupRelations.kt](app/src/main/java/com/kiz9r/expense_tracker/backup/BackupRelations.kt) | Database-independent authentication, strict JSON decoding and cross-record archive validation. |
| [security/LedgerRecovery.kt](app/src/main/java/com/kiz9r/expense_tracker/security/LedgerRecovery.kt) and [ui/features/RecoveryScreen.kt](app/src/main/java/com/kiz9r/expense_tracker/ui/features/RecoveryScreen.kt) | Startup preflight, explicit key-loss recovery into a separate encrypted ledger, durable activation and restart instructions. |
...
| transactions | Account-owned canonical amount/direction/date/timestamp/value date, original merchant/narration/reference/channel, verification/outcome/kind, manual/owned-transfer flags and audit times. Direct account deletion is restricted by the foreign key; the repository explicitly removes dependent records in one transaction before deleting the account. |
...
Reactive account updates clear an invalid account filter and deleted-account statement preview; deleting the final account returns to onboarding. Statement enqueue validates account existence within its write transaction, commits revalidate ownership, and workers cannot recreate deleted staging jobs. No schema change is needed. LedgerIntegrationTest covers scoped cleanup with colliding suffixes, last-account deletion, stale import rejection and rollback using an injected SQL abort. [AccountDeletionUiTest](app/src/androidTest/java/com/kiz9r/expense_tracker/AccountDeletionUiTest.kt) covers cancellation and confirmation through Compose; executed results are recorded in VALIDATION.md.
...
MessageParsers contains modular failure/pending/mandate/reversal/refund/debit/credit classifiers. SMS parser version 2.1.0 delegates conservative field extraction to [SmsFields.kt](app/src/main/java/com/kiz9r/expense_tracker/ingestion/SmsFields.kt). It separates a labeled balance from the movement amount, validates grouping/paise and strict calendar/time values (including AM/PM), preserves sender timestamp versus receipt time, and recognizes UPI/DR/CR and labeled references. Conflicting amounts/accounts/references, mixed debit/credit wording, scheduled/pending activity and unconfirmed refunds/reversals become UNKNOWN with a parseWarning. Observation JSON adds nullable parseWarning and balanceMinor without a Room schema change; older evidence/backups deserialize these as null. Card digits are not treated as account identifiers. The user supplied sms-20260914211524.xml on 14 September after the initial implementation. A host-only SAX audit checks all 299 incoming SBI messages without copying them into fixtures or a ledger: 130 unprefixed UPI debits, 33 compact-date UPI credits, 11 NEFT credits, two linked-account IMPS credits, seven card debits, three CBS credits, one cheque-clearing debit, five NACH execution debits, two cash deposits, one cheque credit, 25 mandate events, 29 scheduled AutoPay reminders, one collect request and 49 informational/promotional messages. [SbiObservedLayouts.kt](app/src/main/java/com/kiz9r/expense_tracker/ingestion/SbiObservedLayouts.kt) handles these SBI-specific field boundaries: amounts without Rs in the known debit/cancellation templates, dMMMyy dates, transfer counterparties before Refno, NEFT remitters before INFO, linked IMPS parties, full UMN/UMRN mandate identifiers, card transaction numbers, cheque references and CBS movement/balance boundaries. The sender allowlist explicitly includes ATMSBI and CBSSBI. Seven card-only alerts have no account identifier and require user assignment even if card/account suffixes coincide. A/C and AC account labels accept longer masked suffixes, keeping only the last four digits in stored evidence. Identical-value mandates with distinct UMNs remain separate; reminder/request messages stay UNKNOWN. Informational filtering retains posted movements even when their merchant contains a promotional term. Other ATM, fee, refund and reversal source wording absent from this sample remains synthetic-only; carrier and phone lifecycle validation is still pending.

SbiSmsReceiver reads only incoming SMS_RECEIVED broadcasts protected by BROADCAST_SMS. [SmsIntake.kt](app/src/main/java/com/kiz9r/expense_tracker/ingestion/SmsIntake.kt) bounds assembly to 64 same-sender parts/16,384 characters, discards OTP/PIN/promotional/unrelated content before persistence, and checks device unlock, actual RECEIVE_SMS permission and the encrypted opt-in setting. goAsync bounds IO to eight seconds and always finishes; malformed intents and intake/scheduling failures cannot escape as receiver crashes. Unique ingestion identity uses normalized sender, sender timestamp and assembled original body; receipt time and parser version do not alter retry identity. Eligible encrypted persistence and the last-received diagnostic timestamp are transactional. No historical SMS scan, raw WorkManager payload, message logging or app-lock gate is introduced.

IngestionWorker drains stored pending observations. Malformed stored JSON/dates are marked processed with a review reason so later messages continue. Startup, boot after unlock, app replacement and the Settings retry action can recover pending work. The recovery receiver uses RECEIVE_BOOT_COMPLETED and credential-protected storage; it does not ingest before the first device unlock. Platform guidance: [Direct Boot](https://developer.android.com/privacy-and-security/direct-boot) and [short broadcast work](https://developer.android.com/develop/background-work/background-tasks/broadcasts).

Settings reports effective tracking status using actual permission refreshed on resume, stored/pending/review SMS counts and the last eligible receipt time. Revocation displays a paused state with an Android permissions link. Needs Review has a source filter, parsed financial context, explicit account and resolution selection, compatible ranked candidates, and a manual-entry path for UNKNOWN events. Ignore does not require an account. Repositories revalidate action/target/account suffix and persist the actual created transaction ID. A late mandate-creation observation cannot overwrite a later cancellation when event times establish ordering; mandate execution alone records spending. Failed to posted evidence updates outcome and kind, and late failure evidence cannot undo a posted transaction.
...
Missing wrapping material, unavailable aliases and database-open failures enter a recovery screen before TrackerViewModel or reactive database subscriptions are created. [LedgerRecovery.kt](app/src/main/java/com/kiz9r/expense_tracker/security/LedgerRecovery.kt) and [RecoveryScreen.kt](app/src/main/java/com/kiz9r/expense_tracker/ui/features/RecoveryScreen.kt) authenticate a portable backup independently of the broken database. After explicit confirmation, recovery restores into a uniquely named encrypted database with a new Keystore-wrapped key, validates and reopens it, then commits the active-database preference. The old database and wrapping material are retained. A failed staging attempt only removes its own temporary database; if pointer activation was attempted, the staged file is retained to avoid deleting a potentially active ledger. The user closes/reopens the app after success so cached dependencies are recreated. Recovery refuses to replace a ledger that has become readable; normal replacement is available in Settings. No automatic reset or destructive migration exists.
...
BackupService snapshots Room transactionally, serializes with Gson in process memory, encrypts and writes through SAF. Oversized plaintext is rejected before key derivation/encryption. A failed output write reports that an incomplete destination must be removed/retried; it never reports success. Password/byte buffers are cleared in finally blocks; immutable Gson/String copies cannot be explicitly zeroed. No plaintext archive is written to disk. Backup input is bounded to 64 MB; logical snapshots still load complete archive data into memory.
...
Backup coverage: [BackupArchiveTest.kt](app/src/test/java/com/kiz9r/expense_tracker/BackupArchiveTest.kt) checks legacy/current decoding, absent or malformed fields, fresh crypto salts/nonces and truncated files. [BackupRecoveryTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/BackupRecoveryTest.kt) covers all-record export/restore, tampering, rejected relationships, output failure/password cleanup, SQL rollback, cancellation during replacement and concurrent enabled SMS intake. [DeviceKeyRecoveryTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/DeviceKeyRecoveryTest.kt) isolates preferences/files to verify retained original ciphertext and a readable replacement. [BackupUiTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/BackupUiTest.kt) exercises onboarding restore, validated preview, cancellation and explicit confirmation; its SAF result is supplied by an instrumentation activity monitor. [BackupReinstallTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/BackupReinstallTest.kt) and [backup-reinstall-fixture.py](app/src/androidTest/backup-reinstall-fixture.py) provide opt-in actual emulator uninstall/reinstall proof with a changed database-key fingerprint and complete record digest. The script requires explicit disposable-emulator acknowledgement and refuses physical-device serials. All fixtures use invented data; the private SMS/PDF samples are not involved.

[SbiObservedLayoutsTest.kt](app/src/test/java/com/kiz9r/expense_tracker/SbiObservedLayoutsTest.kt) contains invented values reproducing 11 bank-layout regression groups. [ProvidedSmsXmlTest.kt](app/src/test/java/com/kiz9r/expense_tracker/ProvidedSmsXmlTest.kt) is opt-in through SBI_SMS_FIXTURE_PATH and streams only incoming SBI SMS from a local XML export using SAX with DTD/external entities disabled. It checks independent expected classifications and financial fields and logs aggregate counts only. The source stays untouched, is excluded by /sms-*.xml, and is never copied into Android storage or committed fixtures. This development validation does not add historical SMS/XML import to the app.

SMS coverage resides in [SmsParserTest.kt](app/src/test/java/com/kiz9r/expense_tracker/SmsParserTest.kt), [SmsIntegrationTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/SmsIntegrationTest.kt), [SmsReviewUiTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/SmsReviewUiTest.kt), and opt-in [SmsReceiverDeliveryTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/SmsReceiverDeliveryTest.kt). [sms-emulator-fixture.py](app/src/androidTest/sms-emulator-fixture.py) sends only synthetic long/repeated/OTP messages to emulator serials; run the opt-in receiver test first and wait for its ready status before injecting. The receiver test checks assembled content over 160 characters, one provisional transaction and no persisted OTP. The emulator raw-PDU command failed to deliver during this run, so the working fixture uses its SMS text command with an alphanumeric sender; no app receiver checks were relaxed.
...
| [AccountDeletionUiTest.kt](app/src/androidTest/java/com/kiz9r/expense_tracker/AccountDeletionUiTest.kt) | Settings account deletion cancellation and confirmation through the real Compose application. |
...
The earlier 14 September SMS sample-validation run passed 45 JVM tests (including the host XML audit). The default Android run reports OK (53 tests): 51 executed passes plus skips for the private PDF and externally injected SMS fixture. Twelve SmsIntegrationTest cases include observed debit-to-statement matching, same-value distinct UMN mandates, explicit card-only account assignment, and NACH/cash movement amounts. The earlier opt-in SMS broadcast pass remains dated evidence; it was not repeated in this run. Lint has zero errors/16 warnings; signed version 1.2.1/code 4 assembled and signature verification passed. The old WorkManager fixture now creates an existing synthetic account because enqueue rejects deleted/missing accounts. Earlier dated PDF/private-sample results remain historical in VALIDATION.md. Stable resume-control tags and waits for loaded content support the asynchronous Compose regression. Test reports/build logs are generated artifacts described in VALIDATION.md. Synthetic tests do not certify actual bank layouts, all delivery conditions or physical-device behavior.

The later 14 September backup/recovery run passed 47 JVM tests with one optional XML skip, and the Android default runner reported OK (65 tests): 62 executed passes and three optional skips. The separate actual emulator reinstall fixture passed both phases, with a different database-key fingerprint and identical complete-record digest. Signed version 1.3/code 5 passed signature verification and cold launch with onboarding restore visible. Lint remains zero errors/16 warnings. Physical-phone, hardware key-invalidation and large-archive/provider checks remain open in leftout.md; exact logs and APK hash are in the latest VALIDATION.md entry.
