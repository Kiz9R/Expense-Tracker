...
The latest 11 September account-deletion run passed 21 JVM tests and 38 executed Android tests (runner reports 39 including the skipped opt-in private-PDF test), lint with zero errors/16 warnings, and signed release assembly/verification. The old WorkManager fixture now creates an existing synthetic account because enqueue rejects deleted/missing accounts. Earlier dated PDF/private-sample results remain historical in VALIDATION.md. Stable resume-control tags and waits for loaded content support the asynchronous Compose regression. Test reports/build logs are generated artifacts described in VALIDATION.md. Synthetic tests do not certify actual bank layouts, all delivery conditions or physical-device behavior.
